...
$name= $phone_no = $address =$email_id="";
$name_err = $phone_no_err = $address_err =$email_id_err=$department_err=$sem_err=$book_name_err=$video_name_err=$issue_err="";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
 ...
 // Validate password
    ...
    if(empty($name_err) && empty($phone_no_err) && empty($address_err)&& empty($email_id_err))
    {
        ...
        $sql = "INSERT INTO staff(name,address,phone_no,email_id) VALUES (?, ?,?,?)";
...
if (isset($_POST['name']) && isset($_POST['phone_no'])&& isset($_POST['address'])&& isset($_POST['email_id']))
    {
        ...
        $query = "INSERT INTO staff (name,address,phone_no,email_id) VALUES ('$name','$address','$phone_no','$email_id')";
        header("Location:http://localhost/vinimaya/options.html");
...
<body>
    <div class="wrapper">
      Registered staff then  <a href="http://localhost/vinimaya/options.html" > continue</a>
        <h3 style="text-align:center; font-family:Times New Roman; font-size:30px;">Staff-details</h3>
        <br/>
